...
**Scout Mode / Scout Reports** (renamed from Lucky Bag Mode, and reframed) — no longer just "guarantee one elite player per position, rest random." The goal is **friction**: right now it's too easy to end up with a squad of great players, and this mode fixes that with a shared, depleting live pool per position plus squad-wide tier caps, delivered turn-by-turn as a curated scout report (one candidate per tier, filtered by afford + cap). **Scoped, 18 July 2026** — full spec in [[Scout Mode Spec]], including a Club Tenets identity layer (soft draw-bias from 1–2 club tenets picked at creation, e.g. league/era/nation/archetype preference) that rides on the same mechanism. Build-ready for sequencing now.
...
5. **Scout Mode / Scout Reports** — was high priority in spirit but unscoped; now scoped (see [[Scout Mode Spec]]) and build-ready. Worth a direct conversation on where it actually slots against 2–4 above, since it's arguably as core a lever as difficulty itself.
...
- ~~Scout Mode / Scout Reports needs a scoping session~~ — **scoped, 18 July 2026.** Reframed as the delivery mechanism for a shared depleting live pool + tier caps, with a Club Tenets identity layer riding on the same draw. Full spec: [[Scout Mode Spec]]. Explicitly kept separate from the club-philosophy perks idea below (Tenets = passive draw-bias, ships with Scout Mode; Perks = active abilities, layers onto Career mode later). No new live open thread from this session — next thing to pin down is where Scout Mode sequences against Career mode / League-era Draft Roulette modifiers / FFP in [[Roadmap]].
